...
public class IntersectSqlExplainInterceptor extends SqlExplainInterceptor {

    private static final Log logger = LogFactory.getLog(IntersectSqlExplainInterceptor.class);
    /**
     * Mysql支持分析SQL的最小版本
     */
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sqlExplain)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    if (!"Using where".equals(extra) &&
                            !extra.matches("Using intersect\\(\\w+,\\w+\\); Using where")) {
